...
function createData(time, name, count, status, price, number, orders, _id) {
    return {
        time,
        name,
        count,
...
function Row({ row, setOrders }) {
    ...
    const handleActionStatus = async (status, id) => {
        try {
            await axios.patch(`/orders/status/${id}`, { status });
            const { data } = await axios('/orders');
            setOrders(data);
            toast(status === 'success' ? 'Purchase confirmed!' : 'Purchase declined!');
        } catch (error) {
                    ...
                    {row.status === 'pending' && (
                        <div className='admin__btns'>
                            <Button
                                ...
                                variant="contained"
                                color="success"
                                onClick={() => handleActionStatus('success', row._id)}
                            >
                                Confirm
                            </Button>
                            <Button
                                ...
                                variant="outlined"
                                color="error"
                                onClick={() => handleActionStatus('cancel', row._id)}
                            >
                                Decline
                            </Button>
                        </div>
    ...
    return (
        <TableContainer>
            <Table aria-label="collapsible table">
                ...
                <TableBody>
                    {orders.map((item) => createData(item.time, item.name, item.orders.length, item.status, item.price, item.number, item.orders, item._id)).map((row) => (
