...
const Card = ({ item, user }) => {
    ...
    const imageUrl1 = `${process.env.REACT_APP_IMAGE_URL}${item.images[0]}`;
    const imageUrl2 = `${process.env.REACT_APP_IMAGE_URL}${item.images[1]}`;
            ...
            <AnimatePresence mode="wait">
                <motion.div
                    ...
                    <Link onClick={() => animateScroll.scrollToTop({
                        ...
                        <img className='catalog__content-img' src={changeImg ? `${process.env.REACT_APP_IMAGE_URL}${item.images[0]}` : `${process.env.REACT_APP_IMAGE_URL}${item.images[1]}`} alt="" />
                    </Link>
                    {
